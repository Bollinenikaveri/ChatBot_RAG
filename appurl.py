...
st.title("🤖 Welcome to RagBot!")
st.caption("Enter a URL, then ask a question.")

st.image("wasteurl.jpg")
...
mal_url = st.text_input("Enter URL:", key="mal_url_input")
...
if mal_url:
    ...
    if st.session_state.retriever and st.session_state.processed_url == mal_url:
        st.markdown("---")
        question = st.text_input("Ask a question about :", key=f"query_{st.session_state.processed_url}")
        if question:
            with st.spinner("🧠 Thinking..."):
                try:
                    ...
                    # Setup LLM
                    llm = ChatGroq(model="meta-llama/llama-4-scout-17b-16e-instruct", temperature=1, api_key=os.getenv("GROQ_API_KEY"))
    ...
    st.info("Enter a URL above to analyze .")
    st.session_state.retriever = None
    st.session_state.processed_url = ""
else:
